...
def copy_if_not_exists(src: Path, dest: Path, relative_path: Path,
                      copied_files: List[Path], copied_dirs: List[Path], skipped: List[Path]) -> None:
    """
    ...
    if src.is_dir():
        ...
        if not dest_exists:
            ...
            try:
                ...
                copied_dirs.append(Path(relative_path))
    ...
    # If source is a file
        ...
        if dest.exists():
            skipped.append(Path(relative_path))
        ...
        try:
            ...
            copied_files.append(Path(relative_path))
